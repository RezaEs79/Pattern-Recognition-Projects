...
# split test and validation data sets
ratio = 0.75
np.random.shuffle(test_array)
max_index = int(test_array.shape[0] * ratio)
splited_test_array = test_array[:max_index, :]
validation_array = test_array[max_index:, :]
...
while(True):
    train_accuracy = my_tree.compute_accuracy(train_array[:,0], my_tree.predict(train_array[:, 1:]))
    test_accuracy = my_tree.compute_accuracy(splited_test_array[:,0], my_tree.predict(splited_test_array[:, 1:]))
