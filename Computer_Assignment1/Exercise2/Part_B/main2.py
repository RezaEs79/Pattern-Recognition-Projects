...
data = pd.read_csv('Liquid-Data_dat.txt', delim_whitespace=True, names=column_names)
...
for i in range(N):
    ...
    if i<NpC1-1:
        partition[0]=1
    elif i>=NpC1-1 and i<NpC1+NpC2-1:
        partition[1]=1
    else:
        partition[2]=1
    ## Maximum Likelihood Estimation
    mu_1,cov_1=Max_likelihood_Estimation(X_train[0:NpC1-1-int(partition[0][0])],NpC1-int(partition[0][0]))
    mu_2,cov_2=Max_likelihood_Estimation(X_train[NpC1-int(partition[0][0]):NpC1+NpC2-1-int(partition[1][0])],NpC2-int(partition[1][0]))
    mu_3,cov_3=Max_likelihood_Estimation(X_train[NpC1+NpC2-int(partition[0][0])-int(partition[1][0]):N-int(partition[2][0])],NpC3-int(partition[2][0]))
